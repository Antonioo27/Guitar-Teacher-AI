...
Supporta la selezione del modello di trascrizione (TabCNN o CRNN).
...
from . import config
from .model_registry import load_model, transcribe, get_available_models
...
class GuitarTutorPipeline:
    ...
    Usage:
        pipeline = GuitarTutorPipeline(model_name="TabCNN")
        result = pipeline.run(
    ...
    def __init__(
        ...
        """
        Inizializza la pipeline caricando il modello scelto.

        Args:
            model_name: Nome del modello ("TabCNN" o "CRNN").
            weights_path: Percorso ai pesi del modello. Se None, usa il default.
        ...
        logger.info(f"Inizializzazione pipeline su device: {self.device}, modello: {model_name}")

        # Carica il modello tramite il registry
    ...
    def run(
        ...
        """
        logger.info("=" * 60)
        logger.info(f"AI GUITAR TUTOR — Pipeline Start (modello: {self.model_name})")
        logger.info("=" * 60)
        ...
        # =====================================================================
        logger.info(f"[Modulo 1+2] Trascrizione audio con {self.model_name}...")
        predicted_notes = transcribe(
            audio_path, self.model, self.model_name, self.device
        )
...
Esempio d'uso:
  python -m guitar_tutor_pipeline.src.app.pipeline \\
    ...
    parser.add_argument(
        ...
        help=f"Modello di trascrizione da utilizzare (default: {config.DEFAULT_MODEL})",
    )
    parser.add_argument(
        "--context", "-c",
        default="",
    ...
    parser.add_argument(
        "--weights", "-w",
        default=None,
        help="Percorso ai pesi del modello (default: auto in base al modello scelto)",
    )
    parser.add_argument(
        "--device", "-d",
        default=None,
    ...
    parser.add_argument(
        "--list-models",
        action="store_true",
        help="Mostra i modelli disponibili ed esci",
    )
    ...
    # Se richiesto, mostra solo la lista dei modelli
    if args.list_models:
        print("\nModelli disponibili:")
        print("=" * 60)
        for m in get_available_models():
            status = "✅" if m["weights_available"] else "❌"
            print(f"  {status} {m['name']}: {m['description']}")
            print(f"     Pesi: {m['weights_path']}")
        print()
        sys.exit(0)
    ...
    try:
        ...
        if not args.audio or not args.reference:
            parser.error("Gli argomenti --audio e --reference sono obbligatori.")
        ...
        if args.output:
            ...
            # Stampa a video
            print("\n" + "=" * 60)
            print(f"RISULTATI DELL'ANALISI (modello: {result['model_used']})")
            print("=" * 60)
